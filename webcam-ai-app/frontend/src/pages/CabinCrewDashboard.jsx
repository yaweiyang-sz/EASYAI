...
import { AlertTriangle, Shield, ShieldOff, Video, Users, PlaneTakeoff, Plane, ShieldAlert, CheckCircle, Clock, Wifi, WifiOff, Maximize2, Camera, Brain } from 'lucide-react';
...
export default function CabinCrewDashboard() {
  ...
  return (
    <div className="h-screen bg-slate-900 text-slate-100 font-sans flex flex-col overflow-hidden">
        ...
        <div className="flex bg-slate-900 rounded-xl p-1 border border-slate-700">
          {[
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors flex items-center space-x-2 ${activeTab === tab.id ? 'bg-slate-700 text-white' : 'text-slate-400 hover:text-white'}`}>
      ...
      {activeTab === 'cameras' && (
        <main className="flex-1 p-4 overflow-y-auto bg-slate-800/30"><CameraManagement /></main>
      )}

      {activeTab === 'ai' && (
        <main className="flex-1 p-4 overflow-y-auto bg-slate-800/30"><AIProcessing /></main>
      )}
      ...
      <main className="flex-1 p-2 flex gap-2 min-h-0 overflow-hidden w-full">
        <div className="flex-1 bg-slate-800 rounded-xl border border-slate-700 p-3 flex flex-col overflow-hidden" style={{flex: 1}}>
          <div className="flex items-center justify-between mb-3 shrink-0">
            <h2 className="font-semibold flex items-center text-sm"><Video className="w-4 h-4 mr-2 text-blue-400" /> Live Camera Feeds</h2>
          ...
          <div className="flex-1 flex flex-col gap-2 overflow-y-auto">
            {loadingCameras ? (
              ...
              cameras.map((camera, index) => (
                <div key={camera.id} className="bg-black rounded-lg border border-slate-700 relative overflow-hidden cursor-pointer hover:border-blue-500 transition-colors"
        ...
        <div className="flex-1 flex flex-col gap-2 min-h-0" style={{flex: 4}}>
          <div className="flex-1 bg-slate-800 rounded-xl border border-slate-700 p-3 flex flex-col min-h-0 overflow-hidden">
            <div className="flex justify-between items-center mb-3 shrink-0">
              <h2 className="font-semibold flex items-center text-sm"><Plane className="w-4 h-4 mr-2 text-indigo-400" /> Cabin Overview (A350 Top View)</h2>
            ...
            <div className="flex-1 bg-slate-900 rounded-lg border border-slate-800 overflow-y-auto overflow-x-hidden">
              <div className="min-h-full">
                <div className="flex flex-col">
                  <div className="bg-red-500/20 border-b-2 border-red-500/50 flex items-center justify-center py-3"><div className="flex items-center space-x-2"><div className="w-3 h-3 bg-red-500 rounded-full animate-pulse"></div><span className="text-xs font-bold text-red-400">COCKPIT</span><div className="w-3 h-3 bg-red-500 rounded-full animate-pulse"></div></div></div>
                  <div className="bg-purple-500/20 border-y-2 border-purple-500/30 flex items-center justify-center py-2"><span className="text-[8px] text-purple-400 font-bold tracking-widest">GALLEY</span></div>
                  ...
                  <div className="flex"><div className="flex-1 bg-yellow-500/20 border-y border-yellow-500/30 flex items-center justify-center py-2 relative"><div className="absolute inset-0 border border-yellow-500/50 animate-pulse"></div><span className="text-[10px] text-yellow-400 font-bold">LAV A</span></div><div className="flex-1 bg-yellow-500/20 border-y border-yellow-500/30 flex items-center justify-center py-2"><span className="text-[10px] text-yellow-400 font-bold">LAV B</span></div></div>
                  ...
                  <div className="flex"><div className="flex-1 bg-yellow-500/20 border-y border-yellow-500/30 flex items-center justify-center py-2"><span className="text-[10px] text-yellow-400 font-bold">LAV C</span></div><div className="flex-1 bg-yellow-500/20 border-y border-yellow-500/30 flex items-center justify-center py-2 relative"><div className="absolute inset-0 border border-yellow-500/50 animate-pulse"></div><span className="text-[10px] text-yellow-400 font-bold">LAV D</span></div></div>
